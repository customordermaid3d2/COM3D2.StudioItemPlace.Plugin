    ...
    public class StudioItemPlace : UnityInjector.PluginBase
    {
        ...
        public void Awake()
        {
            try
            {
                ...
                string[] mpnN = { "帽子", "ヘッドセット", "トップス", "スカート", "ワンピース", "水着", "ブラジャー", "パンツ", "ソックス", "靴", "前髪", "メガネ",
                    "アイマスク", "鼻", "耳", "手袋", "ネックレス", "チョーカー", "リボン", "乳首", "腕", "へそ", "足首", "背中",
                    "しっぽ", "前穴", "右手アイテム", "左手アイテム", "拘束上半身", "拘束下半身", "前穴アイテム", "後穴アイテム"};
                this.mpnL = mpnL;
                this.mpnN = mpnN;
            }
            catch (Exception e)
        ...
        private static void appendIndexFile(string[] menus)
        {
            using (StreamWriter sw = File.AppendText(outputDir + indexPath + indexFile))
            {
                ...
                Debug.Log(logHead + "キャッシュファイル書き込み数:" + i);
            }
        }
        private static void readIndexFile()
        {
            if (indexFileExist())
            {
                Debug.Log(logHead + "キャッシュファイルを読み込みます");
        ...
        private void readAllMenu(int readAmount)
        {
            if (readMenuComplete < GameUty.MenuFiles.Length)
            {
                if (readMenuState == "")
                {
                    readMenuState = "処理準備中(0/2)";
                }
                else if (readMenuState.Contains("処理準備中"))
                {
                ...
                else if (readMenuState.Contains("最終処理中"))
                {
                    ...
                    foreach (KeyValuePair<string, MenuDataInfo> kv in menuDataMap)
                    {
                        ...
                        {
                            Debug.Log("[readAllMenu]カテゴリー名を読み込めなかったためスキップ:" + kv.Key + "," + kv.Value.fullPath);
                        }
                        else
                        {
        ...
        private static byte[] readFileAll(string name, string fullPath = null, bool isMod = true)
        {
            ...
            {
                Debug.LogError("[readFileAll]予期しないファイル読み込みエラー:" + name + "," + fullPath);
            }
            if (b == null || b.Length == 0)
            {
                Debug.LogWarning("[readFileAll]ファイルが読み込めません:" + name + "," + fullPath);
            }
            return b;
        }
        ...
        private void onGUIFunc(int winId)
        {
            try
            {
                ...
                if (placeMenu == 0)
                {
                    GUI.Label(new Rect(430 + 60, 55 + placeSize * 3, 60, placeSize), (viewVanilla ? "表示" : "非表示"), gsLabel);
                    if (GUI.Button(new Rect(430, 55 + placeSize * 3, 60, placeSize), "本体", gsButton))
                    {
                        viewVanilla = !viewVanilla;
                        created = false;
                        placeViewVector = Vector2.zero;
                    }
                    GUI.Label(new Rect(430 + 60, 55 + placeSize * 4, 60, placeSize), (viewMod ? "表示" : "非表示"), gsLabel);
                    ...
                    attachMaid = !GUI.Toggle(new Rect(430, 55 + placeSize * 6, 120, placeSize), !attachMaid, "配置モード", gsToggle);
                    attachMaid = GUI.Toggle(new Rect(430, 55 + placeSize * 7, 120, placeSize), attachMaid, "メイドモード", gsToggle);
                ...
                if (placeMenu == 1)
                {
                    ...
                    if (GUI.Button(new Rect(430, 55 + placeSize * 4, 60, placeSize), "回転", (gizmoType == 1 ? gsButton3 : gsButton)))
                    {
                        gizmoType = 1;
                        if (current != null)
                        {
                            ...
                            if (GUI.Button(new Rect(0, placeSize * i, 380, placeSize), sn, gsButton))
                            {
                                ...
                                using (StreamReader sr = new StreamReader(outputDir + @"\" + saveList[i]))
                                {
                                    while (true)
                                    {
                                        ...
                                        {
                                            Debug.Log("配置プリセット読み込み中エラー:" + sl[7]);
                                        }
                                    }
                                }
                            }
        ...
        private void OnSceneLoaded(Scene scene, LoadSceneMode sceneMode)
        {
            try
            {
                if (useGear && gear == null && scene.name.Contains("SceneTitle"))
                {
                    ...
                    {
                        Debug.LogError("StudioItemPlace:ギアメニューの登録に失敗");
                    }
                }
                removePlacedAll();
                tempTMorph = null;
